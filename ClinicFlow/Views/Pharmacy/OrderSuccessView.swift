                    ...
                    VStack(spacing: 20) {
                        ...
                        VStack(spacing: 20) {
                            ZStack {
                                RoundedRectangle(cornerRadius: 16)
                                    .fill(Color(red: 237/255, green: 241/255, blue: 1.0))
                                    .frame(height: 180)
                                VStack(spacing: 12) {
                                    ZStack {
                                        Circle()
                        ...
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            SummaryRow(label: "Total Amount",
                                       value: "Rs. \(String(format: "%.0f", pharmVM.totalAmount))")
                            SummaryRow(label: "Delivery",
                                       value: pharmVM.selectedDeliveryMethod)
                            SummaryRow(label: "Payment",
                                       value: pharmVM.selectedPaymentMethod == "pharmacy"
                                            ? "At pharmacy"
                                            : pharmVM.selectedPaymentMethod)
                        }
                        .padding(16)
                        .background(Color.white)
                        .cornerRadius(14)
